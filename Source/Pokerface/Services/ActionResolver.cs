...
namespace Pokerface.Services
{
    ...
    {
        public static List<ActionOption> GetLegalActions(PlayerModel player, TableModel tableSettings)
        {
            ...
            int callAmount = tableSettings.CurrentBet - player.CurrentBet;
            ...
            // Bet / Raise
            if (tableSettings.CurrentBet == 0)
            {
                // Bet: min amount is ctx.MinBet
                actions.Add(new ActionOption(EnumPlayerAction.Bet, tableSettings.MinBet));
            }
            else
            {
                // Raise: min amount is either MinBet or enough to match current bet
                int minRaise = Math.Max(tableSettings.MinBet, callAmount);
